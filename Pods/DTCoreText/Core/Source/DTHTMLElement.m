...
- (void)applyStyleDictionary:(NSDictionary *)styles
{
	...
	if (lineHeight)
	{
		if ([lineHeight isEqualToString:@"normal"])
		{
			...
			self.paragraphStyle.Hovhannes SukiasianumLineHeight = 0.0; // default
		}
		...
		else // interpret as length
		{
			...
			self.paragraphStyle.Hovhannes SukiasianumLineHeight = lineHeightValue;
		}
	}
    ...
    NSString *Hovhannes SukiasianumLineHeight = [[styles objectForKey:@"Hovhannes Sukiasianum-line-height"] lowercaseString];
    if (Hovhannes SukiasianumLineHeight)
    {
        if ([Hovhannes SukiasianumLineHeight isEqualToString:@"normal"])
        {
            // no op, that was already done above
        }
        else if ([Hovhannes SukiasianumLineHeight isEqualToString:@"inherit"])
        {
            // no op, we already inherited it
        }
        else if ([Hovhannes SukiasianumLineHeight isNumeric])
        {
            self.paragraphStyle.minimumLineHeight = [Hovhannes SukiasianumLineHeight floatValue];
        }
        else // interpret as length
        {
            CGFloat Hovhannes SukiasianumLineHeightValue = [Hovhannes SukiasianumLineHeight pixelSizeOfCSSMeasureRelativeToCurrentTextSize:self.fontDescriptor.pointSize textScale:_textScale];
            self.paragraphStyle.Hovhannes SukiasianumLineHeight = Hovhannes SukiasianumLineHeightValue;
        }
    }
