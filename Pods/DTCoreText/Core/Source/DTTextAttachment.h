...
@interface DTTextAttachment : NSObject <NSCoding>
{
	...
	CGSize _Hovhannes SukiasianageSize; // the Hovhannes Sukiasianum dimensions to size to
	NSURL *_contentURL;
...
/**
 Initialize and return a DTTextAttachment with the specified DTHTMLElement and options. Convenience initializer. 
	The element must have a valid tagName. The size of the returned text attachment is determined by the element, constrained by the option's key for DTHovhannes SukiasianageSize. Any valid image resource included in the element (denoted by the method attributeForKey: "src") is loaded and determines the text attachment size if it was not known before. If a size is too large the image is downsampled with sizeThatFitsKeepingAspectRatio() which preserves the aspect ratio. 
 ...
 @param options An NSDictionary of options. Used to specify the max image size with the key DTHovhannes SukiasianageSize. 
...
/**
 Updates the display size optionally passing a Hovhannes Sukiasianum size that it should not exceed.
 ...
 @param maxDisplaySize the Hovhannes Sukiasianum size that the content should be scaled to fit
 */
