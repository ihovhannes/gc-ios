...
class RegistrationPasswordView: UIView {
    ...
    lazy var confirmField = UITextField.init()
    ...
    override init(frame: CGRect) {
        ...
        look.apply(Style.registrationPasswordView)
    }
...
extension RegistrationPasswordView {
    ...
    func moveUpComponentsByKeyboard(keyboardHeight: CGFloat, keyboardAnimDuration: TimeInterval) {
        ...
        let passwordsHolderTop = fieldsHolder.convert(CGPoint(x: 0, y: 0), to: self).y
        let desiredPasswordsHolderTop = self.bounds.height - (fieldsHolder.bounds.height + keyboardHeight + CGFloat(LOGIN_HEIGHT + LOGIN_BOTTOM_PAD) + 50)

        var holderTransform = passwordsHolderTop - desiredPasswordsHolderTop
        ...
        UIView.animate(withDuration: keyboardAnimDuration, animations: { [unowned self, holderTransform] () in
            ...
            let passwordsTransform = CGAffineTransform(translationX: 0, y: -1 * holderTransform)
            self.fieldsHolder.transform = passwordsTransform
        })
    }
...
fileprivate extension Style {
    ...
    static var registrationPasswordView: Change<RegistrationPasswordView> {
        return { (view: RegistrationPasswordView) in
    ...
    static var actionButton: Change<UIButton> {
        return { (button: UIButton) -> Void in
            button.setTitle("АКТИВИРОВАТЬ", for: .normal)
