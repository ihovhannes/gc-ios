...
typealias ArchiveSharesViewControllerBindings = (
...
typealias ArchiveSharesViewControllerBindingsFactory = () -> ArchiveSharesViewControllerBindings

class ArchiveSharesViewModel: ReactiveCompatible, DisposeBagProvider {

    fileprivate lazy var offers: Variable<[Offer]> = Variable([])
    lazy var offersCount: Variable<Int64> = Variable(0)
    fileprivate var page: Int = 1

    fileprivate let bindingsFactory: ArchiveSharesViewControllerBindingsFactory
    ...
    required init(bindingsFactory: @escaping ArchiveSharesViewControllerBindingsFactory) {
...
extension ArchiveSharesViewModel {
...
fileprivate extension Reactive where Base == ArchiveSharesViewModel {
    ...
    func detailRouting(tapObservable: Observable<(shareId: Int64?, title: String?, endDate: String?)>) -> Observable<Routing> {
        return tapObservable.map { args in
            return Routing.shareDetail(id: args.shareId, title: args.title, endDate: args.endDate, partnerColor: nil, isArchive: true)
        }
    }
        ...
        return willAppearObservable
                ...
                .flatMapLatest { [unowned base] token -> Observable<OfferListResponse> in
                    return Observable.using({ NetworkService(token: token) }, observableFactory: { service -> Observable<OfferListResponse> in
                        let request: Request<SharesArchiveStrategy> = service.request()
